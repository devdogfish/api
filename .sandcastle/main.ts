...
// Codex auth is copied into /home/agent/.codex so Sandcastle session capture works.
// npm install ensures the sandbox always has fresh Linux-native dependencies.
const hooks = {
  sandbox: {
    onSandboxReady: [
      {
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const issues = plan.output.issues as PlanIssue[];
